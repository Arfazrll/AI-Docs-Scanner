...
export default function VerifySTNK() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch("/api/verify/stnk", {
        method: "POST",
        body: formData,
      });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="border border-dashed border-white/10 rounded-3xl p-8 flex flex-col items-center justify-center bg-zinc-900/30 min-h-[400px] relative transition-all hover:border-white/20 hover:bg-zinc-900/50 group">
              {previewUrl ? (
                ...
                <div className="text-center flex flex-col items-center">
                  ...
                  <p className="font-medium text-xs text-zinc-500 mb-8 max-w-[200px] leading-relaxed">Front side only. JPG, PNG, WEBP (Max 10MB)</p>
            ...
            <div className="text-xs font-medium text-zinc-400 p-4 border border-white/5 rounded-2xl bg-zinc-900/30 flex items-start gap-3 leading-relaxed">
              ...
              <p>Ensure VIN and license plate are clearly visible for cross-validation. Data is processed in real-time and not permanently stored.</p>
            </div>
          ...
          <div className="border border-white/5 p-6 md:p-8 rounded-3xl relative bg-zinc-900/40 backdrop-blur-xl shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1">
              <div className="md:col-span-2">{renderField("nomor_registrasi", "Nomor Registrasi (Plat)")}</div>
              <div className="md:col-span-2">{renderField("nama_pemilik", "Nama Pemilik")}</div>
              <div className="md:col-span-2">{renderField("alamat_pemilik", "Alamat")}</div>
              {renderField("merk", "Merk")}
              {renderField("type", "Type")}
              {renderField("jenis", "Jenis")}
              {renderField("model", "Model")}
              {renderField("tahun_pembuatan", "Tahun Pembuatan")}
              {renderField("isi_silinder", "Isi Silinder")}
              <div className="md:col-span-2">{renderField("nomor_rangka", "Nomor Rangka (VIN)")}</div>
              <div className="md:col-span-2">{renderField("nomor_mesin", "Nomor Mesin")}</div>
              {renderField("warna", "Warna")}
              {renderField("bahan_bakar", "Bahan Bakar")}
              {renderField("warna_tnkb", "Warna TNKB")}
              {renderField("tahun_registrasi", "Tahun Registrasi")}
              <div className="md:col-span-2">{renderField("berlaku_sampai", "Berlaku Sampai")}</div>
            </div>
